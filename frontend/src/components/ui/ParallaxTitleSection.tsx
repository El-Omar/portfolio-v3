...
const ParallaxTitleSection = ({
  ...
  return (
    ...
    >
      <div
        className={twMerge(
          `pointer-events-none h-[50vh] flex items-center justify-center relative opacity-100 transition-opacity duration-300 ${
            isSticky && !reachedBottom
              ? "fixed top-0 left-0 right-0 opacity-50"
              : ""
          } ${reachedBottom ? "absolute top-auto bottom-0 left-0 right-0 opacity-50" : ""} ${titleClassName}`,
        )}
      >
        {title}
      </div>
